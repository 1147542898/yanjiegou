...
class Order extends Base
{
     ...
     *[user_id] => 17
     *[pay_type] => 1
     *[myshop] => [{"shop_id":5,"cart_id":"245","remark_member":"","send_type":"0","freight":""}]
     *
     */
    ...
    public function ptyf()
    {
        ...
        if(empty($cart_id)){
            $this->json_error('购物车参数错误！');
        }
        if(empty($shop_id)){
            $this->json_error('商户参数错误！');
        }
        if(empty($user_id)){
            $this->json_error('用户信息错误！');
        }
        $dispatchprice = 0; //邮费
        //获取用户的地址

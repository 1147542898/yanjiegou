...
class Common extends Controller
{
    ...
    {
        //判断管理员是否登录
        define('URID', session('sinfo.ursid'));
        if (!URID) {
            $this->redirect('shop/login/index');
        }
        define('SHID', session('sinfo.shid'));
        define('BSHID', session('sinfo.bshid'));
        define('GRID', session('sinfo.grid'));
        define('SUPER',session('sinfo.iswho'));
        define('SHNAME', session('sinfo.shname'));
        define('SHORTNAME', session('sinfo.shortname'));
        define('MODULE_NAME',strtolower(request()->controller()));
        define('ACTION_NAME',strtolower(request()->action()));
        //当前操作权限ID
        if(SUPER!=1){
            $urls = Db::name('shopAuthRule')->where(['href'=>MODULE_NAME.'/'.ACTION_NAME,'type'=>2])->value('id');
            if($urls){
    ...
    public function getmenu(){
        ...
        $bsrules = explode(',',session('sinfo.rules'));
        $menus = array();

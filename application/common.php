...
function template_file($module=''){
    ...
    $viewSuffix = config('view_suffix');
    $viewPath = $viewPath ? $viewPath : 'view';
    $filepath = APP_PATH.strtolower(config('default_module')).'/'.$viewPath.'/';
...
function rand_string($len=6,$type='',$addChars='') {
    ...
    switch($type) {
        ...
        default :
            ...
            $chars='ABCDEFGHIJKMNPQRSTUVWXYZabcdefghijkmnpqrstuvwxyz1234567890'.$addChars;
            break;
    }
 ...
 * @param  string $modulename 配置名称
 * @param  string $typename 配置键值
 * +----------------------------------------------------------------------
 */
function get_status($status, $typename = 'status',$modulename = 'system'){
    $statusData = config("$modulename.$typename");
    if (empty($statusData)) {
        return '';
    }
    //判断配置项的状态值是否定义
    ...
    $statusInfo = $statusData[$status];
    //组合html
    $html = '<span class="' . $statusInfo['color'] . '">' . $statusInfo['statusname'] . '</span>';
    return $html;
}
...
function get_status_option($status = 0,  $typename = 'status',$modulename = 'system'){
    $statusData = config("$modulename.$typename");
    if (empty($statusData)) {
        return '';
    }
    ...
    foreach ($statusData as $row) {
        $html .= '<option value="' . $row['status'] . '"';
        $html .= $row['status'] === $status ? ' selected' : '';
        $html .= '>' . $row['statusname'] . '</option>';
    }
    return $html;
}
 ...
 * +----------------------------------------------------------------------
 */
function get_rulerid(){ 
    return intval(session('seadmininfo.aid'));
}
...
/**
    ...
    /** 
    * 计算两组经纬度坐标 之间的距离 
   * params ：lat1 纬度1； lng1 经度1； lat2 纬度2； lng2 经度2； len_type （1:m or 2:km) decimal 小数点; 
   * return m or km 
   */ 
   function GetDistances($lat1, $lng1, $lat2, $lng2, $len_type = 2, $decimal = 4) 
    { 
    ...
    function actionGetDistance($lng1, $lat1, $lng2, $lat2) 
        { 
        ...
        */
        ...
        return round($calculatedDistance/1000); //单位公里
        } 
...
function genTree9($items,$s='son',$p='pid') {
    $tree = array(); //格式化好的树
    foreach ($items as $item)
        if (isset($items[$item[$p]]))
            $items[$item[$p]][$s][] = &$items[$item['id']];
        else
            $tree[] = &$items[$item['id']];
